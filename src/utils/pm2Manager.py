...
async def create_program_pm2(filename, pid, id):
    try:
        if sys.platform == "win32":
            # use run with window
            subprocess.Popen(
                f'pm2 start {filename} -f  --name "{pid}" -- {id}  --restart-delay=10000',
                shell=True,
            ).communicate()
            return 100
        else:
            subprocess.Popen(
                f'sudo pm2 start {filename} --interpreter /usr/bin/python3 -f  --name "{pid}" -- {id}  --restart-delay=10000',
                shell=True,
            ).communicate()
            return 100
...
async def create_device_group_rs485_run_pm2(absDirname, result_rs485_group):
    try:
        ...
        if len(result_rs485_group) > 0:
            result_rs485_list = [
                x
                for i, x in enumerate(result_rs485_group)
                if x["serialport_group"]
                not in {y["serialport_group"] for y in result_rs485_group[:i]}
            ]
            data = []
            for rs485_item in result_rs485_list:
                ...
                for device_item in result_rs485_group:
                    if (
                        rs485_item["serialport_group"]
                        == device_item["serialport_group"]
                    ):
                        item.append(
                            {
                                ...
                                "serialport_stopbits": int(
                                    rs485_item["serialport_stopbits"]
                                ),
                                # Get the first character of the first string
                                "serialport_parity": rs485_item["serialport_parity"][0],
                                # ----- End -----
                                "serialport_timeout": int(
                                    rs485_item["serialport_timeout"]
                                ),
                                "serialport_debug_level": rs485_item[
                                    "serialport_debug_level"
                                ],
                            }
                        )
                data.append(item)
            ...
            for item in data:
                ...
                if id_communication != -1:
                    ...
                    if sys.platform == "win32":
                        # use run with window
                        subprocess.Popen(
                            f'pm2 start {absDirname}/deviceDriver/ModbusRTU.py -f  --name "{pid}" -- "{id}"  --restart-delay=10000',
                            shell=True,
                        ).communicate()
                    else:
                        # use run with ubuntu/linux
                        subprocess.Popen(
                            f'sudo pm2 start {absDirname}/deviceDriver/ModbusRTU.py --interpreter /usr/bin/python3 -f  --name "{pid}" -- {id}  --restart-delay=10000',
                            shell=True,
                        ).communicate()
...
async def restart_pm2_change_template(id_template: int, db: Session):
    try:
        ...
        if result_template:
            if result_template.device_group:
                if hasattr(result_template.device_group[0], "device_list"):
                    result_device_list = [
                        item
                        for item in result_template.device_group[0].device_list
                        if item.status == True
                    ]
                    device_list_rs485 = []
                    device_list_tcp = []
                    ...
                    for item in result_device_list:
                        print(
                            f"{item.id_communication}|{item.id}|{item.name} {item.communication.driver_list.name}"
                        )
                        ...
                        match connect_type:
                            case "Modbus/TCP":
                                pid = (
                                    f"Dev|{id_communication}|{connect_type}|{id_device}"
                                )
        ...
        if device_list:
            upload_channel_query = db.query(uploadChannel_models.Upload_channel).filter(
                uploadChannel_models.Upload_channel.status == 1
            )
            result_upload_channel = upload_channel_query.all()
            if result_upload_channel:
                pid_upload_channel_list = (
                    lambda channel: [f"Log|{item.id}" for item in channel]
                )(result_upload_channel)
