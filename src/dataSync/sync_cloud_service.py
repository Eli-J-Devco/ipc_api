...
from configs.config import DBSessionManager , SyncSetting
...
class SyncData:
    ...
    async def create_message(self ,IdDevice, timeLog):
        ...
        message = {
            ...
            "remainder_all_file": self.number_file,
            "time_sync" : timeLog,
        }
        return message
            ...
            for item in self.list_device_log_file:
                ...
                if result:
                    if typeOfFile == Sync_Setting.Name_Key_File_Log:
                        headers , files ,id_time = await self.create_headers(result)
                        response = self.post_request_log_to_cloud(headers,files,url)
                        await self.update_reponse_logfile_to_db(response,IdChannel,sql_id,id_time)
                    elif typeOfFile == Sync_Setting.Name_Key_URL:
                        arrayjson,array_id_time = await self.create_json(result,IdChannel,sql_id)
                        response = self.post_request_json_to_cloud(arrayjson,url)
                        await self.update_reponse_json_to_db(response,IdChannel,sql_id,array_id_time)
                    elif typeOfFile == Sync_Setting.Name_Key_Ftp:
                        soucre,id_time = await self.create_source(result)
                        response = self.post_request_to_FTPcloud(soucre,url)
                        await self.update_reponse_ftp_to_db(response,IdChannel,sql_id,id_time)
    ...
    async def create_json(self,result,IdChannel,sqlid):
        ...
        for item in result:
            ...
            jsondata = {
                ...
                'serial_number_port': self.serial,
                'datetime': get_utc(),
                'datas': {
