...
from .config import config, orm_provider as db_config
from .devices_entity import ProjectSetup, Devices, PointList
from .devices_model import TopicEnum, PayloadModel, ActionEnum, SLDResponseModel, PointType, PointOutput
...
class MQTTSubscriber(Subscriber):
    ...
    async def get_sld(self, message: PayloadModel):
        ...
        devices = await self.get_devices(session=self.session)
    ...
    @async_db_request_handler
    async def get_points(self, id_template: int, parent: int = None, session: AsyncSession = None):
        ...
        output = []
        for point in points:
            point = PointOutput(**point.__dict__)
            point.children = await self.get_points(id_template, point.id, session=session)
            output.append(point.dict())
        return output

    @async_db_request_handler
    async def get_devices(self, parent: int = None, session: AsyncSession = None):
        query = select(Devices).where(Devices.parent.__eq__(parent))
        result = await session.execute(query)
        devices = result.scalars().all()

        query = select(Devices.id_template).where(Devices.id.__eq__(parent))
        result = await session.execute(query)
        id_template = result.scalars().first()
        points = await self.get_points(id_template, session=session)

        if not devices and not points:
            return []

        output = []
        for device in devices:
            device = SLDResponseModel(**device.__dict__)
            device.children = await self.get_devices(device.id, session=session)
