...
from deviceControl.modeControl.mode_control_service import *
...
LOGGER = setup_logger(module_name='modeControl')
LOGGER.warn(f'--- init ---')
class MainClass:
    # initialize the necessary parameters
    async def start_mqtt_service(self):
        mode_control_instance = ModeControl(LOGGER)
        mqtt_handler_instance = MQTTHandlerModeControl(mode_control_instance)
        ...
        if project_setup_config is not None :
            ...
            mqtt_service.set_topics(mqtt_topics.Control_Setup_Mode_Write_Detail,
