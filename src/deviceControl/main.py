    ...
    try:
        if mqtt_result and 'mode' in mqtt_result and 'offset' in mqtt_result:
            ...
            if mode_auto == 1:
                ...
                print("total_power",total_power + total_wmax_man_temp)
                if value_power_limit_temp is not None :
                    if value_power_limit_temp <= (total_power + total_wmax_man_temp ):
                        value_power_limit = value_power_limit_temp
                        # write information in database 
                        if value_power_limit <= (total_power + total_wmax_man_temp ):
