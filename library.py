...
#########################################################
#using encode module bcrypt 
def hash_password(password):
    salt = bcrypt.gensalt(13)
    hashed_password = bcrypt.hashpw(password.encode("UTF-8"), salt)
    return hashed_password

#using check hash password with password module bcrypt 
def check_password(password, hashed_password):
    return bcrypt.checkpw(password.encode("UTF-8"), hashed_password)

# => dùng mã này không decode được 
#########################################################
...
def encrypt_with_fernet_key(password: str) -> bytes:
    # Mã hóa mật khẩu bcrypt với khóa Fernet
    encrypted_password = client_key.encrypt(password.encode())
    return encrypted_password

def decrypt_with_fernet_key(encrypted_password: bytes) -> str:
    # Giải mã mật khẩu sử dụng khóa Fernet
    decrypted_password = client_key.decrypt(encrypted_password)
    return decrypted_password.decode()

# Sử dụng hàm để mã hóa mật khẩu
matkhau_ki_mahoa = encrypt_with_fernet_key("your_password_here")
print("Mật khẩu đã mã hóa:", matkhau_ki_mahoa)

# Sử dụng hàm để giải mã mật khẩu
matkhau_giai_ma = decrypt_with_fernet_key(matkhau_ki_mahoa)
print("Mật khẩu đã giải mã:", matkhau_giai_ma)
######################################################### 
import binascii
information = input("password here")
information_hex = binascii.hexlify(information).decode()
information = binascii.unhexlify(information_hex)
######################################################### 
