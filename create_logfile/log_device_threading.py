            ...
            for item in time_create_file_insert_data_table_dev:
                ...
                if minute % int_number == 0 and second % 10 == 0:
                    for item in result_all:
                        ...
                        if sql_id not in devices_to_insert:
                            devices_to_insert[sql_id] = {"point_id": point_id, "data": []}

                        devices_to_insert[sql_id]["data"].extend(data_to_write)

            for sql_id, device_data in devices_to_insert.items():
                time_insert_dev = get_utc()
                value_insert = (time_insert_dev, sql_id) + tuple(device_data["data"])
                futures.append(executor.submit(MySQL_Insert_v2, f'dev_{sql_id:05d}', device_data["point_id"], value_insert))
            # Chờ cho tất cả các future hoàn thành
