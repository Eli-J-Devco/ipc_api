...
server_url_register = "http://127.0.0.1:8000/register"
...
username_input = input("Enter the new username:")
password_input = input("Enter the new password:")
# Use the generated salt to hash the password at level 1
hashed_password_lv1 = bcrypt.hashpw(password_input.encode(), salt)
...
print("Salt sent for registration: ", salt_hex)
print("Key sent for registration: ", key_hex)

# Send a POST request to the FastAPI server with JSON data
response = requests.post(server_url_register, json=payload)
